...
- [ ] CLIの全コマンド、引数、オプションが定義されている
- [ ] 正常時と異常時の出力先、文面、終了コードが定義されている
- [ ] プロジェクト名の入力規則と変換結果が一意に決まる
- [ ] 生成先、生成ファイル、各ファイルの役割が定義されている
- [ ] 生成されるCMakeターゲットと利用コマンドが定義されている
- [ ] 既存パスと途中失敗時の外部動作が定義されている
- [ ] 対応環境と配布成果物が定義されている
- [ ] v0.1.0の対象外機能が要件定義書と矛盾しない
- [ ] 要件から外部設計への対応を追跡できる
