...
- [ ] 全モジュールの責務と依存方向が定義されている
- [ ] 主要な型と関数シグネチャが定義されている
- [ ] CLI解析アルゴリズムが外部仕様を満たす
- [ ] 名前検証と正規化アルゴリズムが定義されている
- [ ] テンプレート一覧、プレースホルダー、置換規則が定義されている
- [ ] 既存パスを保護する生成手順が定義されている
- [ ] 途中失敗とロールバックの流れが定義されている
- [ ] OSエラーと終了コードの変換が定義されている
- [ ] Windowsを含むOS差の分離方針が定義されている
- [ ] 単体テスト用の差し替え境界と失敗注入方法が定義されている
- [ ] 上位仕様から内部設計への対応を追跡できる
