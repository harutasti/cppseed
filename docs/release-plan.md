...
- [ ] version、tagおよびRelease名の対応が一意に定義されている
- [ ] 4対象環境とアーカイブ名が定義されている
- [ ] Linux、macOS、Windowsのruntime互換性方針が定義されている
- [ ] 最低コンパイラversionが定義されている
- [ ] アーカイブ内容とSHA256SUMS形式が定義されている
- [ ] release workflowの起動条件、権限、job責務が定義されている
- [ ] 展開後の対象OS別smoke testが定義されている
- [ ] privateからpublicへの移行ゲートが定義されている
- [ ] draft Releaseの手動承認項目が定義されている
- [ ] 失敗、訂正、撤回の方針が定義されている
- [ ] 性能・カバレッジの公開ゲートが定義されている
- [ ] ライセンスと未署名バイナリの扱いが定義されている
- [ ] リリース証跡と完了条件が定義されている
