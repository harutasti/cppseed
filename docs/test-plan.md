...
- [ ] 全機能要件と非機能要件にテストが割り当てられている
- [ ] 単体、コンポーネント、CLI統合、E2Eの責務が分離されている
- [ ] 正常系、境界値、異常系、安全性ケースが定義されている
- [ ] 全ファイル生成段階の失敗注入が定義されている
- [ ] C++17、C++20、C++23の実ビルドが定義されている
- [ ] macOS、Linux、Windowsの環境マトリクスが定義されている
- [ ] 性能とカバレッジの目標が定義されている
- [ ] リリース成果物のsmoke testが定義されている
- [ ] テスト開始条件と完了条件が定義されている
- [ ] 破壊系テストの安全な実行範囲が定義されている
